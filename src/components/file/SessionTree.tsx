...
export default function SessionTree() {
  ...
  // Effective reference: pinned lap, or the first selected lap
  const effectiveRefId = referenceLapId ?? selectedLapIds[0] ?? null;

  if (sessions.length === 0) {
    return <p className={styles.empty}>No sessions loaded</p>;
  }
        ...
        <div key={session.id} className={styles.session}>
          ...
          <div className={styles.laps}>
            ...
            {session.laps.length === 0 && (
              <span className={styles.noLaps}>No complete laps detected</span>
            )}
          </div>
        </div>
      ))}
